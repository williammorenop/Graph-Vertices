...
public class graphVertices<T> {
	...
	public List<T> dfs() {
		...
		for (Aristas arista : vertices.get(0).getVecinos()) {
		...
		while (!pila.isEmpty()) {
			...
			if(!vertices.get(pos).isVisitado())
			{
				...
				resultado.add(top);				
				for (Aristas arista : vertices.get(pos).getVecinos() ) {
					pila.push((T) arista.getDestino());
				}
			}
	 ...
	 * --public boolean hasEdge(T origin, T destination);
	 * 
	 * public Vector<T> dfs (); //pila public Vector<T> bfs (); //cola public
	 * Vector< Pair<T, Double> > dijkstra (); // un nodo a muchos ~ cola de
	 * prioridad public Vector< Pair<T, Double> > floydWarshall (); // de todos a
	 * todos public Vector< T > kruskall (); //arbol de expansion minima - algoritmo
	 * voraz public Vector< T > prim (); //arbol de expansion minima - programacion
	 * dinamica
	 */
}
